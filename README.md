...
| 启动方式 | `gunicorn` | 下拉选择 |
| 项目根目录 | `/www/wwwroot/openclaw` | 代码所在目录 |
| 启动命令 | `app:app -w 1 -b 0.0.0.0:5000 --preload` | 见下方说明 |
| 通信协议 | **WSGI** | Flask 是 WSGI，选这个 |
| 启动用户 | `root` 或 `www` | 默认即可 |
| 安装依赖 | 勾选 | 自动安装 requirements.txt |
| 安装依赖路径 | 留空 | |
| 项目初始化命令 | 留空 | |

**启动命令说明**：宝塔会在前面自动拼上 `gunicorn`，所以只填参数部分：

```
app:app -w 1 -b 0.0.0.0:5000 --preload
```

实际执行的完整命令是：`gunicorn app:app -w 1 -b 0.0.0.0:5000 --preload`

> **`-w 1` 必须保留**，多 worker 会导致任务队列状态混乱。

点击「确认」，项目管理器会自动启动并设置开机自启。
