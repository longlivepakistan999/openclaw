...
| 启动方式 | 选 `gunicorn` |
| 项目根目录 | `/www/wwwroot/openclaw` |
| 启动命令 | `gunicorn -w 1 -b 0.0.0.0:5000 "app:app" --preload` |
| 启动用户 | `www`（默认即可） |
| 安装依赖 | 勾选 |
| 安装依赖路径 | 留空（自动找 requirements.txt） |
| 项目初始化命令 | 留空 |

> **启动命令完整复制**，不要拆分填写，整行就是 `gunicorn -w 1 -b 0.0.0.0:5000 "app:app" --preload`
>
